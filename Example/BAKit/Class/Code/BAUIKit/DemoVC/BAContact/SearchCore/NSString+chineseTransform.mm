...
+ (NSString *)chinese_Pinyin:(NSString *)chineseString{
    ...
    if (BAKit_ObjectIsEmpty(chineseString)) {
        return nil;
    }
    if (BAKit_ObjectIsEmpty(chineseString) || ![BAKit_RegularExpression ba_regularIsChinese:chineseString] ) {
        NSLog(@"所选字符串非中文！（%@）", chineseString);
        return nil;
    }
