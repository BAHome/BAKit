...
- (NSDateComponents *)ba_dateComponentsWithOffsetToDate:(NSDate *)aDate {
    // 如果有特殊需求，可以将此段代码复制出去更改 unitFlags 即可
    unsigned int unitFlags = NSCalendarUnitYear | NSCalendarUnitMonth | NSCalendarUnitDay | NSCalendarUnitHour | NSCalendarUnitMinute | NSCalendarUnitSecond;

    NSDateComponents *dTime = [BAKit_Current_Calendar() components:unitFlags fromDate:self toDate:aDate options:0];
    return dTime;
}
...
- (NSInteger)ba_dateDistanceInDaysToDate:(NSDate *)anotherDate {
    NSDateComponents *components = [self ba_dateComponentsWithOffsetFromDate:anotherDate];
